...
show_help() {
    echo -e "${CYAN}用法:${NC}"
    echo "  ./start_webpre_linux.sh [端口号]"
    echo ""
    echo -e "${CYAN}示例:${NC}"
    echo "  ./start_webpre_linux.sh          # 使用默认端口 10015"
    echo "  ./start_webpre_linux.sh 8080     # 使用端口 8080"
    echo "  ./start_webpre_linux.sh 3000     # 使用端口 3000"
    echo ""
    echo -e "${CYAN}参数:${NC}"
...
if ! $PYTHON_CMD -c "import flask" 2>/dev/null; then
    ...
    if [ $? -ne 0 ]; then
        echo -e "${YELLOW}用户安装失败，尝试系统安装...${NC}"
        sudo $PIP_CMD install -r requirements.txt
        if [ $? -ne 0 ]; then
            echo -e "${RED}错误: 依赖安装失败${NC}"
            echo "请手动安装依赖: $PIP_CMD install -r requirements.txt"
            exit 1
        fi
    fi
else
...
if netstat -tuln 2>/dev/null | grep -q ":$PORT "; then
    ...
    echo "检查占用: netstat -tuln | grep :$PORT"
    echo ""
elif command -v ss &> /dev/null; then
    if ss -tuln | grep -q ":$PORT "; then
        echo -e "${YELLOW}警告: 端口 $PORT 可能已被占用${NC}"
        echo "如果启动失败，请尝试使用其他端口"
        echo "检查占用: ss -tuln | grep :$PORT"
        echo ""
    fi
fi
...
echo "  访问: http://$(hostname -I | awk '{print $1}'):$PORT (局域网)"
echo ""
...
if [ $? -ne 0 ]; then
    ...
    echo "1. 检查端口 $PORT 是否被占用: netstat -tuln | grep :$PORT"
    echo "2. 检查Python依赖是否完整安装"
    echo "3. 检查app.py文件是否存在"
    echo "4. 尝试使用其他端口号"
    echo "5. 检查防火墙设置: sudo ufw status (Ubuntu)"
    echo "6. 检查SELinux设置: getenforce (CentOS/RHEL)"
    echo ""
    exit 1
fi
